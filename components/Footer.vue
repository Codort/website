  ...
  <footer :class="{ 'transparent': isTransparent, 'opaque': !isTransparent }"
    ...
    <div class="px-2 sm:px-6 lg:px-10">
      <div class="flex justify-between">
        <div>
          <ul>
            <li v-for="item in menu" :key="item.path" class="inline">
              <NuxtLink :key="item.path" :to="item.path" class="hover:text-gray-700 pe-2">
                {{ item.name }}
              </NuxtLink>
            </li>
          </ul>
        ...
        <div>
          <div class="flex flex-col items-end">
            <div class="flex space-x-4">
              <NuxtLink v-if="config.socials.github" aria-label="Open github profile"
                class="text-sm text-black transition hover:text-gray-700" target="_blank"
                rel="nofollow noopener noreferrer" :to="config.socials.github">
              ...
              <NuxtLink v-if="config.socials.linkedin" aria-label="Open linkedin profile"
                class="text-sm text-gray-500 transition hover:text-gray-600" target="_blank"
                rel="noopener noreferrer" :to="config.socials.linkedin">
